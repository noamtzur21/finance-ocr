...
export default async function ReceiptsTable(props: { userId: string; showAll: boolean }) {
  const where = { userId: props.userId, type: "expense" as const };
  ...
  return (
    <div className="card p-4">
      <div className="mb-3 text-sm text-zinc-600">
        סה״כ הוצאות (כל הזמן): <span className="font-semibold text-zinc-900">{sum}</span>
      </div>
            ...
            {docs.length === 0 ? (
              <tr>
                <td className="px-3 py-12 text-center text-zinc-600" colSpan={6}>
                  אין קבלות עדיין. <Link className="underline" href="/receipts/upload">העלה קבלה</Link>
                </td>
              </tr>
            ) : (
              docs.map((d) => (
